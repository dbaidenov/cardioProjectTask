...
class App {
  ...
  async _init() {
    ...
    containerWorkouts.addEventListener("click", this._toogleWorkout.bind(this));
  }
  ...
  _newWorkout(e) {
    ...
    //функция для проверки какое именно поле неправильное
    const findWrongInput = function (...inputs) {
    ...
    if (workoutType === "running") {
      ...
      findWrongInput(inputDistance, inputDuration, inputTemp);
      if (
    ...
    //оповещение для удачи
    document.querySelector(".good__luck").style.opacity = 1;
    document.querySelector(".good__luck").style.visibility = "visible";
    setTimeout(function () {
  ...
  //изменять видимость тренировки в сайдбаре
  _toogleWorkout(e) {
    const target = e.target;
    if (target.classList.contains("workout__toggleBtn")) {
      target
        .closest(".workout")
        .querySelectorAll(".workout__details")
        .forEach((value) => value.classList.toggle("workout__details--hidden"));
      target.textContent =
        target.textContent === "Свернуть" ? "Показать" : "Свернуть";
    }
  }
